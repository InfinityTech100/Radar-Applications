...
void camera::handleRequests() {
  ...
  if (recording == 1) {
     ...
     static uint32_t lastCapture = millis();
...
void camera::StartRecording(String fileName) {
  ...
  if (recording == 1) {
    ...
    Serial.println("camera_task::camera.cpp:: Camera start Recording ");
    recording = 1;
    char filename[100];
    sprintf(filename, "/%s.avi", m_fileName);
    ...
    // size_t err = videoFile.write(buf, strlen((const char *)buf));
  }
}
...
void camera::StopRecording() {
  Serial.println("radar_task::camera.cpp:: Stopped Recording");
  videoFile.close();
}
