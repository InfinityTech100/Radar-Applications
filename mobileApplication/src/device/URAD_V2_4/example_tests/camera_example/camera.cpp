...
camera::camera()
{
    //pinMode(21, OUTPUT);   // little red led on back of chip
        ...
        else
        {
            ...
            {   // regular recording
                ...
                {   // we are done the recording
                  //  digitalWrite(21, LOW); // close the file
                    end_avi();
                    frames_so_far = 0;
                    newfile = 0; // file is closed
                    if (repeat > 0)
...
esp_err_t camera::another_pic_avi()
{
    ...
    //digitalWrite(21, HIGH); // red led is on durng the photo and the write
