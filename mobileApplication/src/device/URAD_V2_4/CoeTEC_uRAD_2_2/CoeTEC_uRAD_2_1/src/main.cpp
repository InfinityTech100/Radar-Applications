...
#include "routes/FileTransferRoutes/ft_routes.h"
#include "routes/radarRoutes/urad_routes.h"
#include "radar/uRad.h" 
...
void camera_task(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (fsm == false)
    {
      ...
      {
        String newVideoPath = "IMG_" + String(op_fileCTR) + "_" + String(utcCTr);
        Serial.println("camera_task::Camera capture ");
        OV2640Cam->takePicture(newVideoPath); // Start recording video when object is detected.
      }
    }
    if (fsm == false)
    {
    ...
    else
    {
      ...
      {
        fsm = false;
        Serial.println("radar_task::Camera stop Recording ");
